...
var nextNumb = 0;
  ...
  imageArray[k].style.left=+Math.floor(k%10)*pieceSize*1.5+56+"px";
  //console.log("k="+k+" k%10="+Math.floor(k%10)+" "+(+Math.floor(k%10)*pieceSize+90));
  piecesLeft[k] = +Math.floor(k%10)*pieceSize*1.5+56;//1.324
    if(k<10){
  imageArray[k].style.top=+0*pieceSize+37+"px";
  piecesTop[k] = +1*pieceSize+37;
  }
  else if(k<20){
  imageArray[k].style.top=+0*pieceSize+67+"px";
  piecesTop[k] = +1*pieceSize+67;
  }
  else if(k<30){
  imageArray[k].style.top=+0*pieceSize+37+30*6+"px";
  piecesTop[k] = +1*pieceSize+37+30*6;
  }
  else{
  imageArray[k].style.top=+0*pieceSize*1+37+30*7+"px";
  piecesTop[k] = +1*pieceSize+37+30*7;
  }
  ...
  nextNumb++;
  if(thisNumb==9){thisNumb=0;nextNumb++}
  }
...
function change(n) //make a piece active or deactivate an active piece
{
  ...
  //hopHop(hopperL, hopperT);
    ...
    if(oldIndex>-1&oldIndex!=activeIndex){//oldImage.setAttribute("class", "piece");
      ...
      dummyImage.style.left= dummyL - pieceSize*boardSize/4 + "px";//xpos - relX/2 + "px";
      dummyImage.style.top= dummyT - pieceSize*boardSize/4 + "px";//ypos - relY/2 + "px";
      piecesLeft[dummyIndex]= Math.round((dummyL - pieceSize*boardSize/4)/boardSize);//Math.round((xpos - relX)/boardSize);
      piecesTop[dummyIndex]= Math.round((dummyT - pieceSize*boardSize/4)/boardSize);
      dummyImage.style.zIndex = dummyZ+10;
      //var dummyLoc =

      activeImage = oldImage;//make 1st clicked unit the active image
      activeIndex = oldIndex;

          activeImage.style.left= dummyL + "px";//xpos - relX/2 + "px";
          activeImage.style.top= dummyT + "px";//ypos - relY/2 + "px";
          piecesLeft[activeIndex]= Math.round((dummyL)/boardSize);//Math.round((xpos - relX)/boardSize);
          piecesTop[activeIndex]= Math.round((dummyT)/boardSize);//Math.round((ypos - relY)/boardSize);
        	activeImage.style.borderColor="black";
          activeImage.style.zIndex = dummyZ;
