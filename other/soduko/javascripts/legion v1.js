...
function drawCheckerboard() {
  for (let row = 0; row < gridSizeY; row++) {
    for (let col = 0; col < gridSizeX; col++) {
      ...
      const isOddBlock = blockIndex < 2 || blockIndex > 5 ;//blockIndex % 2 !== 0;
      var colorSet = isOddBlock ? greenBlockColors : mainBlockColors;
      //if(blockIndex > 5){
      if(blockIndex == 7){
            colorSet = purpleBlockColors;}
      else if(blockIndex == 1){
            colorSet = green2BlockColors;}
      else if(blockIndex == 6){
            colorSet = purple2BlockColors;}
...
for (let i = 0; i <= gridSizeY; i++) {
    ctx.beginPath();
    ctx.lineWidth = (i == 2 || i == 0 || i == 8 || i == 6) ? 4 : 1;//(i < 2 || i > 6) ? 4 : 1;
...
function drawCoords() {
  for (let i = 0; i < 10; i++) {
    ...
    ctx.fillText(""+ coordX, i * cellSize+100+cellSize,  9*cellSize+85, 150);
