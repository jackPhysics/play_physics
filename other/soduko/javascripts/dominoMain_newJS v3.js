 ...
 function placePieceHere(event) //move the active piece into a new position
{
  ...
  if(mouseActive==1){
    ...
    if(Math.abs(centreX1-centreX2)<10){flagCentreX = true;
    centreX1 = (centreX2+35)*boardSize;}
    if(Math.abs(centreY1-centreY2)<10){flagCentreY = true;
    centreY1 = (centreY2+35)*boardSize;}
    //alert("left:"+xpos+ " top:"+ypos);
    //console.log("left:"+xpos+ " top:"+ypos);
    if(false){//centre the tile
...
function placePics() //initialise, add the game pieces to the board
{
  ...
  //alert(""+hopperHTMLcode);
      ...
      for(d=0;d<7;d++){
        for(e=0;e<7;e++){
          if(e>=d){
          dominoL[countDominoes]=d;
          dominoR[countDominoes]=e;
          countDominoes++;
          }
        }
      }
      //console.log(dominoL+" "+dominoR)
  ...
  for(var i=0; i<noOfPieces; i++){
    ...
    d1.insertAdjacentHTML('afterend', '<canvas class="piece" id="" width="300" height="600" title="" onmouseover="doThis(this)" onmouseout="doThis2(this)" onclick="change(this)" draggable="true" ondragstart="drag(event)">');
  }
  ...
  var noOfImg=0; //index to hold the number of every piece
	...
	var dumColor = Math.floor(Math.random()*colorOfTiles.length);
  dumColorReal = dumColor;
	colorTiles = colorOfTiles[dumColor];
	colorDots = colorOfDots[dumColor];
	//console.log("col="+dumColor+" "+colorTiles);
  ...
  for(i=0;i<unitTotal+1;i++){
    ...
    imageArray[noOfImg].setAttribute("title", "#"+(noOfImg+1));  //if units have individual names then need "#"+(noOfImg+1)+": "+unitsA[i*noOfItems+1]+" "+(j+1)+"/"+totalNo +unitNamesA[noOfImg]
    armourA[noOfImg]=unitsA[i*noOfItems+4];
...
var nextNumb = 0;
  for(k=0;k<7;k++){
    ...
    if(Math.random()<0.5){
  imageArray[k].style.left=Math.floor(k/9)*pieceSize*2+60+k*40*boardSize+randShift1+"px";
  piecesLeft[k] = Math.round(Math.floor(k/9)*pieceSize*2+60+k*40+randShift1);
  }
    else{
  imageArray[k].style.left=Math.floor(k/9)*pieceSize*2+60+k*40*boardSize-randShift1+"px";
  piecesLeft[k] = Math.round(Math.floor(k/9)*pieceSize*2+60+k*40-randShift1);}
    if(Math.random()<0.5){
  imageArray[k].style.top=(k-Math.floor(k/9)*9)*pieceSize*0+20+randShift2+"px";
  piecesTop[k] = Math.round((k-Math.floor(k/9)*9)*pieceSize*0+20+randShift2);
  }
    else{
  imageArray[k].style.top=(k-Math.floor(k/9)*9)*pieceSize*0+20-randShift2+"px";
  piecesTop[k] = Math.round((k-Math.floor(k/9)*9)*pieceSize*0+20-randShift2);}
  ...
  imageArray[k].style.height=pieceSize*2+"px";
  imageArray[k].style.borderWidth=boardSize*1+"px";//boardSize*2+"px";
  imageArray[k].style.zIndex = 2;//nextZ-1;
		exitA[k]=0;
		imageArray[k].style.transform='rotate('+0+'deg)';
  }
  ...
  for(k=7;k<14;k++){
    ...
    var randShift2 = Math.round(Math.random()*3);
    if(Math.random()<0.5){
  imageArray[k].style.left=55+280*boardSize+randShift1+"px";
  piecesLeft[k] = Math.round(55+280+randShift1);
  }
    else{
  imageArray[k].style.left=55+280*boardSize-randShift1+"px";
  piecesLeft[k] = Math.round(55+280-randShift1);}
    if(Math.random()<0.5){
  imageArray[k].style.top=60+(k-7)*35*boardSize+randShift2+"px";
  piecesTop[k] = Math.round(60+(k-7)*35+randShift2);
  }
    else{
  imageArray[k].style.top=60+(k-7)*35*boardSize-randShift2+"px";
  piecesTop[k] = Math.round(60+(k-7)*35-randShift2);}
  ...
  imageArray[k].style.zIndex = 2;//nextZ-1;
	//console.log(exitA[k]);
		exitA[k]=270;
		imageArray[k].style.transform='rotate('+270+'deg)';
	//console.log(exitA[k]);
  }
  ...
  for(k=14;k<21;k++){
    ...
    var randShift2 = Math.round(Math.random()*3);
    if(Math.random()<0.5){
  imageArray[k].style.left=Math.floor(0)*pieceSize*2+20+(21-k)*40*boardSize+randShift1+"px";
  piecesLeft[k] = Math.round(Math.floor(0)*pieceSize*2+20+(21-k)*40+randShift1);
  }
    else{
  imageArray[k].style.left=Math.floor(0)*pieceSize*2+20+(21-k)*40*boardSize-randShift1+"px";
  piecesLeft[k] = Math.round(Math.floor(0)*pieceSize*2+20+(21-k)*40-randShift1);}
    if(Math.random()<0.5){
  imageArray[k].style.top=(k-Math.floor(0)*9)*pieceSize*0+(8)*40+randShift2+"px";
  piecesTop[k] = Math.round((k-Math.floor(0)*9)*pieceSize*0+(8)*40+randShift2);
  }
    else{
  imageArray[k].style.top=(k-Math.floor(0)*9)*pieceSize*0+(8)*40-randShift2+"px";
  piecesTop[k] = Math.round((k-Math.floor(0)*9)*pieceSize*0+(8)*40-randShift2);}
  ...
  imageArray[k].style.zIndex = 2;//nextZ-1;
		exitA[k]=180;
		imageArray[k].style.transform='rotate('+180+'deg)';
  }
  ...
  for(k=21;k<28;k++){
    ...
    var randShift2 = Math.round(Math.random()*3);
    if(Math.random()<0.5){
  imageArray[k].style.left=20+0*boardSize+randShift1+"px";
  piecesLeft[k] = Math.round(20+0+randShift1);
  }
    else{
  imageArray[k].style.left=20+0*boardSize-randShift1+"px";
  piecesLeft[k] = Math.round(20+0-randShift1);}
    if(Math.random()<0.5){
  imageArray[k].style.top=30+(28-k)*35*boardSize+randShift2+"px";
  piecesTop[k] = Math.round(30+(28-k)*35+randShift2);
  }
    else{
  imageArray[k].style.top=30+(28-k)*35*boardSize-randShift2+"px";
  piecesTop[k] = Math.round(30+(28-k)*35-randShift2);}
  ...
  imageArray[k].style.borderWidth=boardSize*1+"px";//boardSize*2+"px";
  imageArray[k].style.zIndex = 2;//nextZ-1;
  changeRotR(imageArray[k], k, 1);
		exitA[k]=90;
		imageArray[k].style.transform='rotate('+90+'deg)';
  }
...
function change(n) //make a piece active or deactivate an active piece
{
  ...
  //hopHop(hopperL, hopperT);
    ...
    mouseActive=0;
    //doEdgeColor();
    activeImage = dummyImage;//make 1st clicked unit the active image
...
function changeSize() //change the size of board and pieces following key press
{
  ...
  imageArray[k].style.height=(pieceSize*boardSize*2/pieceShrink)+"px";
  imageArray[k].style.borderWidth=boardSize*1+"px";//boardSize+"px";//*2+"px";
  //if(k==20){
