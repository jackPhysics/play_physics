...
var noOfColumns = 4;
var maxColors = 4;
var maxColorsRepeat = 4;//really means no repeat
  ...
  for (let row = 0; row < gridSizeY; row++) {
    for (let col = 0; col < gridSizeX; col++) {
      ...
      if (num !== 0 && Math.random() < 0.4) { // 40% chance to display
        ctx.fillText(num, col * cellSize + cellSize / 2+leftMargin, row * cellSize + cellSize / 2+leftMargin);
        board50pc[row][col] = 1;
      }
    }
  }
