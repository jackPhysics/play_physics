...
class nucleusViewer extends Canvas implements Runnable
{
	...
	public void nextNucleus(int p, int n, int i)
	{
		...
		int dumdum=1;
		if(Parent.stable)dumdum=0;
		mother.graph.addData(pdash, ndash, dumdum);
	}
	...
	public void nextNucleus()
	{
		...
		mother.chsData.addItem(Parent.name+"-"+Adash);
		mother.history.append(Parent.name+"-"+Adash+"\n");
		int dumdum=1;// 1 means unstable
		if(Parent.stable)dumdum=0; //0 means stable
		mother.graph.addData(pdash, ndash, dumdum);	
	}
	...
	public void init()
	{
		...
		offscreenImage=createImage(647,260);//mother.centrePanel.getSize().width,mother.centrePanel.getSize().height);
