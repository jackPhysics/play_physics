...
function plotNewFrame(){
    ...
    var old_r1 = r1;
      ...
      ctx.fillText("moon below horizon", canvW/6, 5*canvH/6);}
        ...
        percMoon = illum/100;
        indexMoon = 1/(1-2*percMoon);//n = 1/(1-2p)
        distOffMoon = r1/2*(indexMoon-1/indexMoon);//d = r/2(n - 1/n)
        if(indexMoon<0){
          sign = -1;
        ...
        bet1 = Math.PI/2 - alp1;
        th3 = th1 + bet1*sign*sign2;//start angle point
        th4 = th3 + 2*alp1*sign*sign2;//end angle point
        //SMALL MOON
        //moonCol = "white";
        //moonNot = "black";
        ctx.beginPath();
        ...
        ctx.beginPath();
        //moonCol = "white";//sunCol;//whatSky;
        if(sign<0){moonNot = moonCol};//moonCol
      ...
      if(moonDay<=15.765294){
        sign2=-1;
        arcCW=true;
        imgMoon.src = "moon4R.png";
      }
      else{
        sign2=1;
        imgMoon.src = "moon4L.png";
        arcCW=false;
      }
      //if(arcCW){sign=-1*sign;}
      d1 = sign2*sign*distOffMoon;
      ...
      //imgMoon.src = "moon4.png";
      // Draw image to canvas
      //  ctx.drawImage(imgMoon, x1-r1, y1-r1);
      if(fullFlag){
      ctx.arc(x1, y1, r1, 0, 2*Math.PI, arcCW) //ctx.arc(objPosB, canvH/2, objRadB, 0, 2 * Math.PI);
      ctx.fill();
      imgMoon.src = "moon4.png";
