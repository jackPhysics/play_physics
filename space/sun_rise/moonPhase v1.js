...
function plotNewFrame(){
  ...
  moonWE = minX+multiplier*(timeNowMoon);
    ...
    ctx.fillText("moon below horizon", canvW/6, 5*canvH/6);}
      ...
      if(fullFlag){
      ctx.arc(x1, y1, r1, 0, 2*Math.PI) //ctx.arc(objPosB, canvH/2, objRadB, 0, 2 * Math.PI);
      ctx.fill();
      }
      else{
      ctx.arc(x1, y1, r1, th1, th2) //ctx.arc(objPosB, canvH/2, objRadB, 0, 2 * Math.PI);
      ctx.fill();
