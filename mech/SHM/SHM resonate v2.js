...
/*    ctx.beginPath();
    ...
    ctx.stroke();*/
    document.getElementById("myRangeM").value=10;
...
function plotDriver(){
  ...
  //    stopDriver();}
//** driver osc
    var dum1 = 2*Math.PI*time/driverT;
    dum1 = Math.sin(dum1);
    driverDisp = driverAmp*dum1;
    var driverDisp2 = driverDisp + canvW/2;
    moveData = moveData + " xd="+driverDisp;
//** object osc
    objForce = -springK*objDisp + driverDisp*drIntensity;
    objAcc = objForce/objMass;
    moveData = moveData + " Fo="+objForce;
    objVel = objVel*(1-dampPerc/100) + objAcc*deltaTime/1000;
    objDisp = objDisp + objVel*deltaTime/1000;
    //objDisp = objDisp*objScale;
    if(objDisp>objDispMax){objDisp=objDispMax;}
    if(objDisp<-objDispMax){objDisp=-objDispMax;}
    moveData = moveData + " xo="+objDisp+"\n";
    var objDisp2 = objDisp + canvW/2;

    if(objDisp2<maxL){maxL=objDisp2;
      if(canvW/2-maxL>maxA){maxA=canvW/2-maxL;}
      document.getElementById("dummy2").innerHTML="max Amp<br><br>"+ Math.round(maxA/5*10)/10+ " mm";;//"max Amp = "+ Math.round(maxA/5*10)/10+ " mm";
    }
    if(objDisp2>maxR){maxR=objDisp2;
      if(maxR-canvW/2>maxA){maxA=maxR-canvW/2;}
      document.getElementById("dummy2").innerHTML="max Amp<br><br>"+ Math.round(maxA/5*10)/10+ " mm";;//"max Amp = "+ Math.round(maxA/5*10)/10+ " mm";
    }
          ...
          //***OBJECT
              ...
              ctx.lineWidth = "1";
              ctx.fillStyle = "brown";
              ctx.arc(objDisp2, canvH/2, 50, 0, 2 * Math.PI);
              ctx.fill();
          //***OBJECT centre
                  ctx.beginPath();
                  ctx.lineWidth = "1";
                  ctx.fillStyle = "black";
                  ctx.arc(objDisp2, canvH/2, 2, 0, 2 * Math.PI);
                  ctx.fill();
                        ...
                        //**right marker
                            ...
                            ctx.strokeStyle = "blue";
                            ctx.moveTo(maxR, 50);
                            ctx.lineTo(maxR, canvH-50);
                            ctx.stroke();
//**DRIVER
          ctx.beginPath();
          ctx.lineWidth = "1";
          ctx.fillStyle = "yellow";
          ctx.arc(driverDisp2, canvH/2, 5, 0, 2 * Math.PI);
          ctx.fill();
          //**centre marker
              ctx.beginPath();
              ctx.setLineDash([10, 10]);
