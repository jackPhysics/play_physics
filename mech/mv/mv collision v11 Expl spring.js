...
*/
    ...
    document.getElementById("myRangeE").value=20;
    document.getElementById("myRangeAS").value=50;
...
if(objVelA<velAMin){
  ...
  //**AVERAGING
/*
  objAccA=(oldForce+springForce)/2/objMassA;
  objAccB=(oldForce+springForce)/2/objMassB;
  objVelA=objVelA-(oldAccA+objAccA)/2*deltaTime/1000;
  objVelB=objVelB+(oldAccB+objAccB)/2*deltaTime/1000;
  objDispA=(oldVelA+objVelA)/2*deltaTime/1000;
  objDispB=(oldVelB+objVelB)/2*deltaTime/1000;
  springX=springX+(oldDispA+objDispA)/2+(oldDispB+objDispB)/2;
*/
  /*if(firstGoFlag){
    objVelA=0;
    objVelB=0;
  ...
  objVelA=objVelA-(objAccA)*deltaTime/1000;
  objVelB=objVelB+(objAccB)*deltaTime/1000;
  objDispA=(objVelA)*deltaTime/1000;
  objDispB=(objVelB)*deltaTime/1000;
  springX=springX+(objDispA)+(objDispB);
  springForce=springX*springK;
/*
  keA2 = 0.5*objMassA*Math.pow(objVelA, 2);
  keB2 = 0.5*objMassB*Math.pow(objVelB, 2);
  keT2 = keA2+keB2;
  newEnergy = newEnergy-keT2;*/
  //var springX = -2*startEnergy/springForce;
  //springK = -2*newEnergy/(Math.pow(springX, 2));

  oldForce = springForce;
  oldAccA = objAccA;
  oldAccB = objAccB;
  oldVelA = objVelA;
  oldVelB = objVelB;
  oldDispA = objDispA;
  oldDispB = objDispB;
  //alert("oldAccA=" + objAccA+ " oldAccB=" + objAccB+ "  oldVelA=" + objVelA+ "  oldVelB=" + objVelB+"  oldDispA=" + objDispA+ "  oldDispB=" + objDispB);
if(springX>=0){//if(springX>=0){//if(newEnergy<=0){
    collisionFlag = false;
    afterCollFlag = true;
    objBumpA= 0;
    objBumpB= 0;
    ...
    function changeUB(n){
      ...
      //plotNewMoveQ()
        ...
        function changeE(){
          if(makeChangesFlag){
          //n2 = parseInt(document.getElementById("myRange2").value)/100;//valueAsNumber(slider2.value);
          startEnergy = parseInt(document.getElementById("myRangeE").value);//valueAsNumber(slider2.value);
          startEnergy = startEnergy/20;
          startEnergy = Math.pow(10, startEnergy);
          document.getElementById("dummyE").innerHTML="E = "+printNumber(startEnergy)+" J";
        ...
        function changeAS(){//accuracy v speed
          if(makeChangesFlag){
          //n2 = parseInt(document.getElementById("myRange2").value)/100;//valueAsNumber(slider2.value);
          ...
          /*dummyAS = dummyAS/10;
              ...
              startForce = dummyAS;//dummyAS2;
              springForce = startForce;
              document.getElementById("dummyAS").innerHTML="&lt;-Acc/Sp-&gt; = x"+printNumber(springForce);
              */
          deltaTime = dummyAS;
            document.getElementById("dummyAS").innerHTML="&lt;-Acc/Sp-&gt; = x"+printNumber(deltaTime);
              //alert(""+percComp);
          goFlag=false;
          plotDriver();
          //document.getElementById("dummy3").innerHTML="m = "+objMass+" kg<br>k = "+springK+" N/m<br>";
