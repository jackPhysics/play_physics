...
window.onload= function(){
  ...
  ctx.fillRect(48, 95, 504, 210);//504
...
var maximum = 0;
var volume = 0;
var looper = 7;//should be odd number?
...
//var j=2;
  ...
  if(i==0){diff1=-1*yVal;}//diff calculations are to make sure ends of string do not move
  if(i==length){maximum=Math.abs(yVal0);}//volume based on last value
  //else if(i==length){diff2=-1*yVal;}
    diff2=0;
  var yVal0 = yVal+(diff1*(length-i)+diff2*(i))/length;//yVal+(diff1*(length-i)+diff2*(i))/length;
  //var yVal0 =yVal;
  //if(i==0){
  //ctx.moveTo(50, 200-yVal0);}
  //if(Math.abs(diff1)>Math.abs(diff2)){diffX=Math.abs(diff1)}else{diffX=Math.abs(diff2)}
  diffX=Math.abs(diff1);
  if(dampingFlag){
  ...
  //if(Math.abs(yVal0)>maximum){maximum=Math.abs(yVal0);}//volume based on largest value
  yValR = yVal0;
...
volume = Math.pow(maximum*2/side,2)*300;
//alert(""+maximum);
var volOffset = -50;
