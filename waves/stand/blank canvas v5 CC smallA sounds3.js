...
var plotPoints = new Array();
  ...
  const freqSlider = document.getElementById("myRange1");
...
volume2 = ((volume/175)+1)/2;
//document.getElementById("dummy2").innerHTML=""+volume+" <br>"+volume2;
...
// Convert slider position to logarithmic frequency
...
*/
  ...
  const frequency = n1*n2;//Math.pow(10, minLog + scale * (maxLog - minLog));
...
function updateFrequency() {
  ...
  //freqDisplay.textContent = frequency.toFixed(1);

  if (oscillator) {
    oscillator.frequency.setValueAtTime(freq2, audioContext.currentTime);
  }
}
...
function updateVolume() {
  ...
  if (gainNode) {
    gainNode.gain.setValueAtTime(volume2, audioContext.currentTime);
  }
}
...
startButton.addEventListener("click", function() {
  if (!isPlaying) {
    ...
    oscillator.frequency.value = sliderToFrequency(freq2);
    gainNode.gain.value = Number(volume2);
    ...
    //document.getElementById("startButton").value="Stop Sound";
  } else {
    oscillator.stop();
    ...
    //document.getElementById("startButton").value="Start Sound";
  }
});
