...
//  if(incAngle-Math.PI/2<Math.PI/2){//less than 90
    var refr1 = n1*Math.sin(incAngle)/n2;
    if(refr1<1&&refr1>-1){
        var refr2 = Math.asin(refr1);
      tirFlag = false;}
    else if(refr1>1||refr1<-1){
      refr2=Math.PI-incAngle;//rayAngle-5*Math.PI/2;
      tirFlag = true;
      //alert(""+incAngle);
    }
    else{
      ...
      valB1=256;
        ...
        var ctx = c.getContext("2d");
                ...
                ctx.stroke();
                      ...
                      grd.addColorStop(0, "rgb("+valR2+","+valG2+","+valB2+")");//"cyan"
                      grd.addColorStop(highlightPos, "white");
                      grd.addColorStop(1, "rgb("+valR2+","+valG2+","+valB2+")");//"cyan"
                      // Fill with gradient
                      ctx.fillStyle = grd;
                      ...
                      grd1.addColorStop(0, "rgb("+valR1+","+valG1+","+valB1+")");//"cyan"
                      grd1.addColorStop(highlightPos, "#ddddff");//"ivory" "white"
                      grd1.addColorStop(1, "rgb("+valR1+","+valG1+","+valB1+")");//"cyan"
                      // Fill with gradient
                      ctx.beginPath();
                      ctx.fillStyle = grd1;
                      ctx.arc(canvW/2, canvH/2, canvH/2-30, 0, 1*Math.PI, true);
...
//plot norm N text after calc mirror inclination
  ...
  var data9s = " rayRY="+rayRY;
      ...
      data6s = data6.toFixed(2)+" deg";
      data7s = data7.toFixed(2)+" deg";}
      else{
      data6s = data6.toFixed(1)+" deg";}
      //testing
      ...
      if(tirFlag){
      document.getElementById("dummy2").innerHTML="angle of reflection<br>="+data7s;}//data6s
      else{
      document.getElementById("dummy2").innerHTML="angle of refraction<br>="+data7s;}
      //document.getElementById("dummy3").innerHTML="meet angle<br>"+(meetAngle/Math.PI*180).toFixed(0)+"<br>incAngle<br>"+((meetAngle - Math.PI/2)/Math.PI*180).toFixed(0);

      var n1s=n1.toFixed(2);
      var n2s=n2.toFixed(2);
      document.getElementById("med1").innerHTML="medium 1: n="+n1s;
      document.getElementById("med2").innerHTML="medium 2: n="+n2s;
